   ...
   while($row = mysqli_fetch_assoc($res))
    {
      ...
      $status = "<button onclick='toggle_Status($row[id],0)' class='btn btn-dark btn-sm shadow-none'>active
      </button>";

      if(!$row['status']){
        $status = "<button onclick='toggle_Status($row[id],1)' class='btn btn-danger btn-sm shadow-none'>inactive
        </button>";
      }
